...
if cfgs.dataset == 'SemiMETRLA':
    ...
    train_data = SemiPeMS(mode='train',
                        split_train=cfgs.split_train,
                        split_val=cfgs.split_val,
                        num_timesteps=cfgs.num_timesteps)
    val_data = SemiPeMS(mode='val',
                      split_train=cfgs.split_train,
                      split_val=cfgs.split_val,
                      num_timesteps=cfgs.num_timesteps)
...
train_loader = DataLoaderX(train_data, batch_size=cfgs.batch_size, shuffle=True, num_workers =6, pin_memory=True)
val_loader = DataLoaderX(val_data, batch_size=cfgs.batch_size, shuffle=False, num_workers =6, pin_memory=True)
test_loader = DataLoaderX(test_data, batch_size=cfgs.batch_size, shuffle=False, num_workers =6, pin_memory=True)


"""
# only for DCRNN and STAWNET
train_loader = DataLoaderX(train_data, batch_size=cfgs.batch_size, shuffle=True)
val_loader = DataLoaderX(val_data, batch_size=cfgs.batch_size, shuffle=False)
...
def train(epoch, logger,train_loader, val_loader, net,optimizer, A_wave, loss_criterion, max_speed, edges, edge_weights):
    ...
    for batch_idx, (past_data, current_data, future_target) in enumerate(train_loader):

        net.train()
        optimizer.zero_grad()


        X_batch, y_batch = current_data, future_target
        X_batch = X_batch.to(device=args.device)
        y_batch = y_batch.to(device=args.device)


        loss_params = dict(
            model=net, x_natural=X_batch.unsqueeze(2) ,A_wave=A_wave, edges = edges, edge_weights= edge_weights, y=y_batch ,
        ...
        _, train_predict, train_target = eval_val(train_loader,
                                                      net, A_wave,
                                                      edges,
                                                      edge_weights,
                                                      loss_criterion)
        ...
        if batch_idx % cfgs.log_interval == 0:
            logger_info(logger, distributed,
                        'Epoch: {} [{}/{} ({:.0f}%)]\tTrain Loss: {:.4f}  Val Loss: {:.4f} MAE: {:.4f}  time:{:.3f}'.format(
                            epoch, batch_idx * len(current_data), samples_total,
...
def eval_val(val_loader,net, A_wave, edges, edge_weights, loss_criterion):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (past_data, current_data, future_target) in enumerate(val_loader):
            ...
            out = net(X_batch.unsqueeze(2),A_wave, edges, edge_weights)
...
def eval_val_pgd(val_loader, net, A_wave, edges, edge_weights, loss_criterion, max_speed, cfgs, y_future_batch=None):
    """
    Trains one epoch with the given data.
    :param training_input: Training inputs of shape (num_samples, num_nodes,
    num_timesteps_train, num_features).
    ...
    with torch.no_grad():
        ...
        epoch_val_losses = []
        adv_pgd_val_predict = []
        for batch_idx, (data, target, future_target) in enumerate(val_loader):
            ...
            _, data_current_pgd, index = _ST_pgd_whitebox(net,
            ...
            adv_pgd_out = net(data_current_pgd, A_wave, edges, edge_weights)


            loss = loss_criterion(current_out, data_future_batch)
            epoch_val_losses.append(loss.detach().cpu().numpy())
            val_predict.append(current_out.cpu().detach().numpy())
            val_target.append(data_future_batch.cpu().detach().numpy())
            adv_pgd_val_predict.append(adv_pgd_out.cpu().detach().numpy())
...
def main():
    ...
    if cfgs.backbone == 'GWNET':
        ...
        model = agcrn(device, num_nodes,  input_dim=input_dim, rnn_units=rnn_units, output_dim=output_dim, 
    ...
    if args.mode == "TRAIN":
        ...
        _, _, val_target, val_adv_predict, _ = eval_val_pgd(val_loader, model, A_wave, edges,
        ...
        model.eval()
        _, test_clean_predict, test_target, test_adv_predict, _ = eval_val_pgd(test_loader, model, A_wave, edges,
                                                              edge_weights, loss_criterion, max_speed,
                                                              cfgs)
